...
export const generateSalaryReport = async (req, res) => {
    try {
        ...
        doc.fontSize(16).font('Helvetica').text(`Salary Report ${month ? `- ${month}/${year}` : ''}`, { align: 'center' }).moveDown();
        
        // Add headers
        doc.fontSize(10);
        doc.text('Employee', 50, 100);
        doc.text('Month/Year', 150, 100);
        doc.text('Basic', 220, 100);
        doc.text('OT', 270, 100);
        doc.text('EPF', 310, 100);
        doc.text('ETF', 350, 100);
        doc.text('Deduct', 390, 100);
        doc.text('Net', 440, 100);
        doc.text('Status', 490, 100);
        ...
        salaries.forEach((salary, index) => {
            if (y > 700) {
                ...
                doc.text('Employee', 50, 50);
                doc.text('Month/Year', 150, 50);
                doc.text('Basic', 220, 50);
                doc.text('OT', 270, 50);
                doc.text('EPF', 310, 50);
                doc.text('ETF', 350, 50);
                doc.text('Deduct', 390, 50);
                doc.text('Net', 440, 50);
                doc.text('Status', 490, 50);
                
                y = 70;
            }
            
            doc.text(salary.employeeId.name.substring(0, 12), 50, y);
            doc.text(`${salary.month}/${salary.year}`, 150, y);
            doc.text(`$${salary.basicSalary}`, 220, y);
            doc.text(`$${salary.overtimePay}`, 270, y);
            doc.text(`$${salary.epf}`, 310, y);
            doc.text(`$${salary.etf}`, 350, y);
            doc.text(`$${salary.deductions}`, 390, y);
            doc.text(`$${salary.netSalary}`, 440, y);
            doc.text(salary.status, 490, y);
